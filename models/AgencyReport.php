...
class AgencyReport
{
    ...
    public function getProfitabilityPerDeparture($agencyId, $startDate = null, $endDate = null)
    {
        $sql = "SELECT 
                    ...
                    s.costo_transporte,
                    s.costo_otros,
                    (s.costo_guia + s.costo_transporte + s.costo_otros) as costo_total_operativo,
                    COALESCE(SUM(rd.subtotal), 0) as ingreso_bruto,
                    (COALESCE(SUM(rd.subtotal), 0) - (s.costo_guia + s.costo_transporte + s.costo_otros)) as utilidad_neta,
                    s.cupos_totales,
